    ...
    public void onEntityDied(LivingDeathEvent pEvent) {
        try {
            if (!(pEvent.source.getSourceOfDamage() instanceof EntityPlayer)
                    || pEvent.source.getSourceOfDamage() instanceof FakePlayer)
                return;
            else {
                ...
                if (tHeldItem != null) {
                    ...
                    for (String s : AngerMod._cfgManager.ButcherItems) {
                        if (tUnlocItemName.contains(s.toLowerCase())) return; // Player used defined butcher-item to
                ...
                if (!MathHelper.FlipTheCoin(_mCfg.KamikazeChance)) return; // Head, you win
                else {
                    ...
                    tW.createExplosion(
                            pEvent.entityLiving,
                            pEvent.entityLiving.posX,
                            pEvent.entityLiving.posY,
                            pEvent.entityLiving.posZ,
                            1.5F,
                            flag);
                }
            }
            ...
            AngerMod.Logger.warn(
                    "KamikazeRevenge.onEntityDied.Error",
                    "An error occoured while processing onEntityDied. Please report");
