    ...
    public MinecraftBlock(String pFullQualifiedBlockName, int pDimensionID) {
        ...
        if (tSplitFQBN.length < 2) {
            AngerMod.Logger.error(
                    String.format(
                            "BlockName %s in config is invalid. Make sure you use full [domain]:[blockname] notation!",
                            _mFQBN));
            ...
            if (tSplitFQBN.length > 2) {
                ...
                if (IntHelper.tryParse(tMetaData)) _mBlockMetaData = Integer.parseInt((String) tMetaData);
                else _mBlockMetaData = -1;
            }
        }
    }
        ...
        try {
            if (tPlayerDIM == _mDimensionID) {
                ...
                if (tBlockDomain.modId.equalsIgnoreCase(_mBlockDomain)) // BlockDomain ( ModID ) matches
                {
                    ...
                    } else AngerMod.Logger.debug("_mBlockName match");
                } else AngerMod.Logger.debug("_mBlockDomain mismatch");
            } else AngerMod.Logger.debug(String.format("DimensionID mismatch %d != %d", tPlayerDIM, _mDimensionID));
        } catch (Exception e) {
